...
BASE_URL = "http://127.0.0.1:8000/api/rutas"
...
def test_crear_ruta():
    ...
    response = requests.post(f"{BASE_URL}/ruta/", json=data, headers=headers)
...
def test_listar_rutas():
    token = test_login_usuario()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/ruta/", headers=headers)
